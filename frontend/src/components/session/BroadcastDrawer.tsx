...
import { Instagram, Facebook, Youtube, Radio, RotateCw, Square, ExternalLink, X } from 'lucide-react';
import type { BroadcastDestination, BroadcastLike, BroadcastPlatform, BroadcastStatus } from '@/components/session/BroadcastTypes';
import { libelleStatut, selectionnable, nbSelectionnes, formatDuree } from '@/lib/broadcastUi';
...
const Ligne: React.FC<{ d: BroadcastDestination; b: BroadcastLike }> = ({ d, b }) => {
  const url = d.status === 'not_connected' || d.status === 'reauth' ? b.connectUrl(d.platform) : null;
  const pendantLive = b.live;
  return (
    <li className="flex items-center gap-3 py-2.5" data-testid={`broadcast-${d.platform}`} data-broadcast-status={d.status}>
      <span className={`shrink-0 ${d.status === 'live' ? 'text-[var(--bt-accent)]' : 'text-white/80'}`}>{ICONES[d.platform]}</span>
      <span className="min-w-0 flex-1">
        <span className="block text-sm text-white/90 leading-tight">{d.label}</span>
      ...
      {/* Hors direct : interrupteur (compte relié) ou action de connexion. */}
      ...
      {!pendantLive && url && (
        <a
          ...
          className="inline-flex items-center gap-1 text-[11px] text-white/60 hover:text-[var(--bt-accent)] shrink-0"
          data-testid={`broadcast-connect-${d.platform}`}
        >
          {d.status === 'reauth' ? 'Reconnecter' : 'Connecter'} <ExternalLink className="w-3 h-3" />
        </a>
      )}
...
export const BroadcastDrawer: React.FC<BroadcastDrawerProps> = ({ broadcast, open, onClose, mobile = false }) => {
  ...
  useEffect(() => { if (!open) setConfirmerArret(false); }, [open]);
  useEffect(() => {
    if (!open) return;
  ...
  const liste = (
    <ul className="divide-y divide-white/10" data-testid="broadcast-liste">
      {lignes.map((d) => <Ligne key={d.platform} d={d} b={broadcast} />)}
    </ul>
  );
  ...
  const principal = broadcast.live ? (
    ...
    >
      <Radio className="w-4 h-4" /> Démarrer le direct
    </button>
  );
