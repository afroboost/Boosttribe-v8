    ...
    <div className={`group max-w-[78%] flex flex-col ${mine ? 'items-end' : 'items-start'}`}>
      ...
      <div
        className={`px-3 py-2 rounded-2xl text-sm leading-snug break-words whitespace-pre-wrap ${
          mine
            ? 'bg-gradient-to-br from-[#8A2EFF] to-[#FF2FB3] text-white rounded-tr-sm'
  ...
  return (
    <div className="flex items-end gap-2 p-2.5 border-t border-white/10 bg-black/30">
      <textarea
        ...
        className="flex-1 resize-none max-h-28 px-3 py-2 rounded-xl bg-white/8 border border-white/10 text-white text-sm placeholder-white/30 focus:outline-none focus:border-[#8A2EFF]/60 focus:ring-1 focus:ring-[#8A2EFF]/40"
      />
      <button
        onClick={send}
        disabled={!value.trim()}
        className="flex-shrink-0 p-2.5 rounded-xl bg-gradient-to-br from-[#8A2EFF] to-[#FF2FB3] text-white disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition-opacity"
        title="Envoyer"
      >
        <Send size={18} />
      </button>
