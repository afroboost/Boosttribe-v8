...
const AFRO = { gradient: 'linear-gradient(135deg, var(--bt-accent) 0%, var(--bt-accent-2) 100%)', pink: 'var(--bt-accent-2)' };
type Fmt = '9:16' | '16:9';
...
export const PromoEditor: React.FC<PromoEditorProps> = ({ sessionId, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-3 bg-black/90 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-4xl max-h-[92vh] overflow-y-auto rounded-2xl border-2 border-[rgb(var(--bt-accent-rgb)/0.4)] bg-[#15151b] p-5 shadow-2xl"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
                ...
                <div className="mt-1.5 grid grid-cols-2 gap-2">
                  {(['9:16', '16:9'] as Fmt[]).map((f) => (
                    <button key={f} onClick={() => setFormat(f)}
                      className={`p-2.5 rounded-xl border text-sm font-medium ${format === f ? 'border-[var(--bt-accent)] bg-[rgb(var(--bt-accent-rgb)/0.1)] text-white' : 'border-white/15 text-white/60'}`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setIsPaid(false)} className={`p-2.5 rounded-xl border text-sm font-medium ${!isPaid ? 'border-[var(--bt-accent)] bg-[rgb(var(--bt-accent-rgb)/0.1)] text-white' : 'border-white/15 text-white/60'}`}>Gratuite</button>
                <button onClick={() => setIsPaid(true)} className={`p-2.5 rounded-xl border text-sm font-medium ${isPaid ? 'border-[var(--bt-accent)] bg-[rgb(var(--bt-accent-rgb)/0.1)] text-white' : 'border-white/15 text-white/60'}`}>Payante</button>
              </div>
              {isPaid && (
                <div className="space-y-2">
                  ...
                  <label className="flex items-center gap-2 text-sm text-white/80 cursor-pointer">
                    <input type="checkbox" checked={allowAccessRequests} onChange={(e) => setAllowAccessRequests(e.target.checked)} className="accent-[var(--bt-accent)]" />
              ...
              <label className="flex items-center gap-2 text-sm text-white/80 cursor-pointer">
                <input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="accent-[var(--bt-accent)]" />
      ...
      {cropSrc && (
        <div className="fixed inset-0 z-[160] bg-black/95 flex flex-col" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="p-4 bg-black/90 flex items-center gap-3 flex-wrap justify-center">
            ...
            <input type="range" min={1} max={3} step={0.01} value={zoom} onChange={(e) => setZoom(Number(e.target.value))} className="w-40 accent-[var(--bt-accent)]" />
